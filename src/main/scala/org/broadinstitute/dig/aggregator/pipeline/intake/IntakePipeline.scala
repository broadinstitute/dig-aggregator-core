...
object IntakePipeline extends Pipeline {
  ...
  private def createDummyProcessor(processorName: Name, config: BaseConfig): Processor = {
    new Processor(processorName, config) {
      ...
      override def getWork(opts: Processor.Opts): IO[Map[String, Seq[Run.Result]]] = IO(Map.empty)
      ...
      override def processOutputs(outputs: Map[String, Seq[Run.Result]]): IO[_] = IO.unit
