...
final class Consumer(opts: Opts, val topic: String) extends LazyLogging {
  ...
  /**
   * All the partitions for this topic.
   */
  ...
  private def updateState(state: State, ref: Ref[IO, Option[Consumer.Records]]): IO[Unit] = {
    ...
    wait >> ref.modify(None -> _).flatMap {
      ...
      case Some(records) => {
        val updated = state ++ records

        // in debug mode, don't actually write the state
        val io = if (opts.noWrite()) IO(updated) else updated.save(xa)

        // recurse after updating
        io.flatMap(updateState(_, ref))
      }
    }
  }
